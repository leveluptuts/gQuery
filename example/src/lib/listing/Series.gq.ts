...
export type LatestSeriesQuery = { __typename?: 'Query', latestPlaylist?: { __typename?: 'Playlist', _id: string, slug: string, title?: string | null | undefined } | null | undefined };
...
// Cached
export async function getLatestSeries({ fetch, variables }: GGetParameters<LatestSeriesQueryVariables>, options?: CacheFunctionOptions) {
	const data = await g.fetch<LatestSeriesQuery>({
		queries: [{ query: LatestSeriesDoc, variables }],
		fetch
	})
	await latestSeries.set({ ...data, errors: data?.errors, gQueryStatus: 'LOADED' })	
	return data
}
